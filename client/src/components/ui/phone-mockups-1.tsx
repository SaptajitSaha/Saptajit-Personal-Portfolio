...
export function PhoneCarousel({ images, className = "" }: PhoneCarouselProps) {
  ...
  const autoplayPaused = hoverPaused || focusPaused || touchPaused || pageHidden || reducedMotion;

  const updateProgress = useCallback((elapsed: number) => {
    const progress = Math.min(1, elapsed / AUTOPLAY_DELAY);
    progressRef.current?.style.setProperty("stroke-dashoffset", String(DOT_PROGRESS_CIRCUMFERENCE * (1 - progress)));
  }, []);
