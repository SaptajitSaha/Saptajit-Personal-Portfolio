...
export function PhoneCarousel({ images, className = "" }: PhoneCarouselProps) {
  ...
  const select = useCallback((index: number) => setActiveIndex(current => (index + images.length) % images.length), [images.length]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => select(activeIndex + 1), AUTOPLAY_DELAY);
    return () => window.clearTimeout(timer);
  }, [activeIndex, autoplayPaused, images.length, select]);
    ...
    <section ref={carouselRef} className={`phone-carousel ${className}`} data-autoplay={autoplayPaused ? "paused" : "playing"} data-focus-paused={focusPaused || undefined} aria-roledescription="carousel" aria-label="Nidarr mobile product screens" onKeyDown={onKeyDown} onFocusCapture={() => setFocusPaused(true)} onBlurCapture={onBlurCapture} onPointerEnter={onPointerEnter} onPointerLeave={onPointerLeave} tabIndex={0}>
      ...
      <div className="phone-carousel__stage" aria-hidden="true">
        {images.map((image, index) => {
          ...
          return <figure className="phone-carousel__phone" data-slot={slot} key={image.src}><div className="phone-carousel__speaker" /><img src={image.src} alt="" width="440" height="871" loading={index === 0 ? "eager" : "lazy"} /></figure>;
        })}
      </div>
